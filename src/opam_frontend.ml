...
let create_bundle cli global_options conf f =
  let conffile =
  ...
  @@ fun gt ->
     OpamSwitchState.with_ `Lock_read gt
     @@ fun st ->
        let env = OpamPackageVar.resolve ?opam:None ?local:None st in
        let conf = normalize_conf env conf conffile in
        let package =
          try OpamSwitchState.find_installed_package_by_name st conf.conf_package
        ...
        let changes : string list =
          OpamPath.Switch.changes gt.root st.switch conf.conf_package
        ...
        OpamFilename.with_tmp_dir @@
          ...
          let copy_embedded (type a) (module F : System.FILE_INTF with type t = a) path dst_base =
          ...
          let (embedded_dirs : (basename * dirname) list),
            ...
            List.fold_left (fun (dirs, files) -> function
                | Copy_alias (dirname, alias) when Sys.is_directory dirname ->
                   let dir =  copy_embedded (module System.DIR_IMPL) dirname alias in
                   (dir::dirs, files)
                | Copy_alias (filename, alias) ->
                   let file = copy_embedded (module System.FILE_IMPL) filename alias in
                   (dirs, file::files)
                | Copy_opam path ->
