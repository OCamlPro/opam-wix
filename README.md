...
[opam-wix](https://github.com/OCamlPro/opam-wix) is a cygwin plugin for [opam](https://opam.ocaml.org/), the OCaml package manager. This plugin provides integration with the WiX Toolset, allowing you to create Windows Installer packages (MSI) for your OCaml applications and packages.
...
1. Make sure you have opam installed. If not, you can install it by following the instructions at [opam's website](https://opam.ocaml.org/doc/Install.html).
2. You will also need to had installed `util-linux` package on Cygwin.
...
```bash
WIXPATH='/cygdrive/c/Program Files (x86)/<your installation directory>'
```
4. Install plugin with opam :
```bash
...
To generate MSI file for an executable 'e' within package 'p' run:

```bash
opam-wix p -b e --wix-path $WIXPATH
```

To generate MSI file for an executable from the file system 'epath' boxed in 'p', run:

```bash
opam-wix p --bp epath --wix-path $WIXPATH
```

For customize interface purpose you can override some default graphic components like main icon :
```bash
opam-wix p -b e --wix-path $WIXPATH --ico "new path to icon"
```

**Note**:
- Chosen executable shouldn't be a script!
- Since $WIXPATH often contains spaces, it is fine if program will print the warning about it.
